$(document).ready(function() {
	...
	populateMarkup(numItems, headerArray, contentArray, type);
...
function populateMarkup(numItems, headerArray, contentArray, type) {

	let mediaMarkup = ``;
	switch (type) {
		case "image":
			mediaMarkup = `<div class="floatingImg" style="background-image:url(assets/images/${imageFilename}1.jpg)"></div>`;
			break;
		case "video":
			mediaMarkup = `<div class="videoContainer textAndVideo square side" style="display:none">
				<video class="video-js" controls preload="auto" width="1024" height="768" data-setup="{}" onloadeddata="fadeInVideoFrame()" onplaying="stopAllAudio()">
				  <source src="assets/video/${videoFilename}1.mp4" type="video/mp4">
				</video>
			</div>`
