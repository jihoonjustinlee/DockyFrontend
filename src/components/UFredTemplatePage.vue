    ...
    <transition name="bounce-in" enter-active-class="animated bounceInRight">
      <div class="container" :key="type" v-if="isMounted">
        ...
        <div class="view" :key="active_el">
          <div :class="['appearance', {background: applyBackground(type)}]" v-if="active_el === 0">
            <iframe class="preview" :src="'/assets/ufred/'+type+'.html'" frameborder="0" :key="type"></iframe>
          </div>
          ...
          <div class="details" v-if="active_el === 3">
            <p>Docky Compatibility: 
              <font-awesome-icon :icon="dockyCompatible(type)"/>
            </p>
...
export default {
  ...
  methods:{
    ...
    dockyCompatible(type){
      const notCompatible = [
        'drag_and_drop',
        'table'
      ]

      for (let i=0; i<notCompatible.length; i++){
        if (type === notCompatible[i]){
          return "ban"
        }
      }
      return "check"
...
<style scoped>
  ...
  .appearance.background{
    background-image: url('/assets/ufred/interface/images/background_2048.jpg');
  }
